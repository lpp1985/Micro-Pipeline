...
ALL_GO = open( '%s/gene_association.goa_uniprot'%(GO_Location),'rU'  )
ALL_GO.next()
go_id = Ddict()
id_go = Ddict()
go_def = {}
DEF = open( '%s/NAME_DEF.list'%(GO_Location),'rU'  )
change_go = File_Ddict( open(  '%s/relationship.alter'%(GO_Location),'rU'  )  ).read(1,2)
for line in DEF:
	line_l = line.split('\t')
...
print(go_id)
son_father = File_Ddict( open('%s/relationship.son'%(GO_Location),'rU' ) ).read(2,1)
all_stop = File_Ddict(  open('%s/ROOT.root'%(GO_Location) ,'rU') ).read(1)
root_static = Ddict()
...
END = open( sys.argv[2]+'.GO-mapping.list','w'  )
...
END_DETAIL = open( sys.argv[2]+'.GO-mapping.detail','w'  )
